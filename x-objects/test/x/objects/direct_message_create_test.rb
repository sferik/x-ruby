...
module X
  class DirectMessageCreateTest < Minitest::Test
    ...
    def test_create
      ...
      message = DirectMessage.create(to: User.new({"id" => "8"}), text: "yo", client: @client, attachments: [{media_id: "3"}])
    ...
    def test_create_with_id
      ...
      assert_equal "2", DirectMessage.create(to: 8, text: "yo", client: @client).id
    end

    def test_create_without_data
      @client.stub(:post, "dm_conversations/with/8/messages", {"errors" => []})

      assert_nil DirectMessage.create(to: "8", text: "yo", client: @client)
    end

    def test_referenced_posts_skip_references_without_id
      message = DirectMessage.new({"id" => "1", "referenced_tweets" => [{}, {"id" => "5"}]})

      assert_equal ["5"], message.referenced_posts.map(&:id)
    end
    ...
    def test_create_without_conversation_id
      ...
      assert_nil DirectMessage.create(to: "8", text: "yo", client: @client).dm_conversation_id
    end

    def test_create_without_event_id
      @client.stub(:post, "dm_conversations/with/8/messages", {"data" => {"dm_conversation_id" => "9-8"}})

      assert_raises(ArgumentError) { DirectMessage.create(to: "8", text: "yo", client: @client) }
    end
