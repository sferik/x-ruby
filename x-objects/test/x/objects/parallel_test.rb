  ...
  module Objects
    class ParallelTest < Minitest::Test
      ...
      def test_map_preserves_order
        results = Parallel.map([3, 1, 2], concurrency: 3) do |value|
          sleep(value / 100.0)
          value * 2
        end
      ...
      def test_map_with_empty_items
        assert_empty Parallel.map([], concurrency: 4) { |value| value }
      end

      def test_map_accepts_enumerables
        assert_equal [2, 4], Parallel.map(1..2, concurrency: 4) { |value| value * 2 }
      end

      def test_map_runs_in_other_threads
        threads = Parallel.map([1, 2], concurrency: 2) { Thread.current }
      ...
      def test_map_raises_block_errors
        error = assert_raises(RuntimeError) do
          Parallel.map([1, 2], concurrency: 2) { |value| raise "boom #{value}" if value.eql?(2) }
        end
      ...
      def test_map_does_not_report_exceptions_on_stderr
        _, err = capture_subprocess_io do
          Parallel.map([1], concurrency: 1) { raise "boom" }
        rescue RuntimeError
          nil
        end
      ...
      def test_map_leaves_global_exception_reporting_alone
        ...
        Parallel.map([1], concurrency: 1) { |value| value }
