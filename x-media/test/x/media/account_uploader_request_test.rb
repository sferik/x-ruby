...
module X
  class AccountUploaderRequestTest < Minitest::Test
    ...
    def test_upload_profile_image_binary_body
      AccountUploader.upload_profile_image_binary(client: @client, content: CONTENT, boundary: BOUNDARY)
    ...
    def test_update_profile_image_sends_file_content
      AccountUploader.update_profile_image(client: @client, file_path: PNG_FILE, boundary: BOUNDARY)

      assert_equal image_body(File.binread(PNG_FILE), BOUNDARY), @request.body.b
    end

    def test_upload_profile_image_binary_default_boundary
      AccountUploader.upload_profile_image_binary(client: @client, content: CONTENT)

      assert_equal image_body(CONTENT, request_boundary), @request.body.b
    end

    def test_update_profile_image_default_boundary
      AccountUploader.update_profile_image(client: @client, file_path: PNG_FILE)

      assert_equal image_body(File.binread(PNG_FILE), request_boundary), @request.body.b
    end

    def test_upload_profile_banner_binary_body_without_dimensions
      AccountUploader.upload_profile_banner_binary(client: @client, content: CONTENT, boundary: BOUNDARY)
    ...
    def test_upload_profile_banner_binary_body_with_dimensions
      AccountUploader.upload_profile_banner_binary(client: @client, content: CONTENT, width: 1500, height: 500,
    ...
    def test_update_profile_banner_sends_file_content_and_dimensions
      AccountUploader.update_profile_banner(client: @client, file_path: PNG_FILE, width: 1500, height: 500,
        offset_left: 10, offset_top: 20, boundary: BOUNDARY)
    ...
    def test_upload_profile_banner_binary_default_boundary
      AccountUploader.upload_profile_banner_binary(client: @client, content: CONTENT)

      assert_equal banner_body(CONTENT, request_boundary), @request.body.b
    end

    def test_update_profile_banner_default_boundary
      AccountUploader.update_profile_banner(client: @client, file_path: PNG_FILE)
