...
module X
  class MediaUploaderRequestTest < Minitest::Test
    ...
    def test_upload_binary_body_and_headers
      MediaUploader.upload_binary(client: @client, content: CONTENT, media_category: "tweet_image", boundary: BOUNDARY)
    ...
    def test_upload_sends_file_content
      MediaUploader.upload(client: @client, file_path: GIF_FILE, media_category: "tweet_gif", boundary: BOUNDARY)

      assert_equal upload_body(File.binread(GIF_FILE), "tweet_gif", BOUNDARY), @request.body.b
    end

    def test_upload_binary_default_boundary
      MediaUploader.upload_binary(client: @client, content: CONTENT, media_category: "tweet_image")

      assert_equal upload_body(CONTENT, "tweet_image", request_boundary), @request.body.b
    end

    def test_upload_default_boundary
      MediaUploader.upload(client: @client, file_path: GIF_FILE, media_category: "tweet_gif")
    ...
    def test_upload_binary_rejects_invalid_category_before_requesting
      assert_raises(ArgumentError) { MediaUploader.upload_binary(client: @client, content: CONTENT, media_category: "bogus") }
      assert_not_requested(:post, UPLOAD_URL)
    end

    def test_upload_rejects_missing_file_before_reading_it
      error = assert_raises(RuntimeError) { MediaUploader.upload(client: @client, file_path: "nope.jpg", media_category: "tweet_image") }
