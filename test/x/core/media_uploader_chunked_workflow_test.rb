...
module X
  class MediaUploaderChunkedWorkflowTest < Minitest::Test
    ...
    def test_default_media_type_is_inferred_from_the_file
      stub_workflow
      MediaUploader.chunked_upload(client: @client, file_path: "test/sample_files/sample.png", media_category: "tweet_image")
    ...
    def test_missing_file_is_rejected_before_requesting
      error = assert_raises(RuntimeError) do
        MediaUploader.chunked_upload(client: @client, file_path: "nope.mp4", media_category: "tweet_video")
      end
    ...
    def test_invalid_category_is_rejected_before_requesting
      assert_raises(ArgumentError) do
        MediaUploader.chunked_upload(client: @client, file_path: VIDEO_FILE, media_category: "bogus", media_type: "video/mp4")
      end
    ...
    def test_media_without_id
      ...
      error = without_thread_reports do
        assert_raises(KeyError) { MediaUploader.chunked_upload(client: @client, file_path: VIDEO_FILE, media_category: "tweet_video") }
      end
    ...
    def test_segment_files_and_directories_are_removed
      ...
      directories = track_temporary_directories do
        MediaUploader.chunked_upload(client: @client, file_path: VIDEO_FILE, media_category: "tweet_video", chunk_size_mb: 0.0625)
      end
      ...
      directories = track_temporary_directories do
        without_thread_reports do
          assert_raises(InternalServerError) do
            MediaUploader.chunked_upload(client: @client, file_path: VIDEO_FILE, media_category: "tweet_video")
          end
        end
      end
    ...
    def test_cleanup_removes_an_emptied_directory
      ...
      MediaUploader.send(:cleanup_file, path)
