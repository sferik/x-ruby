...
module X
  class RateLimitHandlerTest < Minitest::Test
    ...
    def test_defaults
      handler = RateLimitHandler.new

      assert_equal [0, 900], [handler.max_rate_limit_retries, handler.max_rate_limit_wait]
    end

    def test_returns_what_the_block_returns
      assert_equal :done, handle(RateLimitHandler.new) { :done }
    end

    def test_retries_nothing_by_default_and_reads_no_reset
      error = assert_raises(TooManyRequests) { handle(RateLimitHandler.new) { refuse(headers: {"x-rate-limit-remaining" => "0"}) } }
    ...
    def test_retries_after_waiting_for_the_reset
      result = handle(RateLimitHandler.new(max_rate_limit_retries: 2)) { (@attempts < 2) ? refuse(reset_in: 30) : @attempts += 1 }

      assert_equal [3, [30, 30]], [result, @sleeps]
    end

    def test_raises_the_error_once_the_retries_run_out
      handler = RateLimitHandler.new(max_rate_limit_retries: 2)
    ...
    def test_raises_at_once_when_the_limit_resets_after_the_maximum_wait
      handler = RateLimitHandler.new(max_rate_limit_retries: 3, max_rate_limit_wait: 60)
      handler.max_rate_limit_wait = 10

      assert_raises(TooManyRequests) { handle(handler) { refuse(reset_in: 11) } }
      assert_equal [1, []], [@attempts, @sleeps]
      assert_raises(TooManyRequests) { handle(RateLimitHandler.new(max_rate_limit_retries: 1, max_rate_limit_wait: 10)) { refuse(reset_in: 10) } }
      assert_equal [3, [10]], [@attempts, @sleeps]
    end

    def test_a_refusal_without_a_reset_time_waits_a_minute_and_doubles_the_wait
      handler = RateLimitHandler.new(max_rate_limit_retries: 5, max_rate_limit_wait: 240)
