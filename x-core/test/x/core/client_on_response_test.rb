...
module X
  class ClientOnResponseTest < Minitest::Test
    ...
    def test_on_response_receives_every_response
      stub_request(:get, "https://api.twitter.com/2/users/me").to_return(body: {data: {id: "1"}}.to_json, headers: {"x-rate-limit-remaining" => "74", "x-rate-limit-limit" => "75", "x-rate-limit-reset" => "1"})
      @client.get("users/me")

      assert_equal [[:get, "https://api.twitter.com/2/users/me", 200, 1]], @responses.map { |response| [response.http_method, response.uri.to_s, response.status, response.resource_count] }
      assert_equal 74, @responses.first.rate_limit.remaining
    end

    def test_on_response_receives_a_failed_response_before_the_error
      stub_request(:post, "https://api.twitter.com/2/tweets").to_return(status: 429, headers: {"x-rate-limit-remaining" => "0"})
    ...
    def test_on_response_is_optional_and_can_be_set_later
      stub_request(:delete, "https://api.twitter.com/2/tweets/1")
      client = Client.new
