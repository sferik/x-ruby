...
module X
  class MediaUploaderProcessingStatusTest < Minitest::Test
    cover MediaUploader
    ...
    def test_media_without_id
      error = assert_raises(KeyError) { MediaUploader.await_processing(client: @client, media: {}) }
    ...
    def await
      MediaUploader.stub(:sleep, ->(seconds) { @sleeps << seconds }) do
        MediaUploader.await_processing(client: @client, media: {"id" => TEST_MEDIA_ID})
      end
    end
