...
It handles OAuth 1.0a, OAuth 2.0 with token refresh, and bearer tokens. It also handles redirects, proxies, timeouts, HTTP errors, rate limits, and streaming. It depends only on the `simple_oauth` gem, which has no dependencies of its own. Media uploads live in [`x-uploader`](../x-uploader).
