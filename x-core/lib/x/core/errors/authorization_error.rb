...
module X
  ...
  # used, or an app's API key and secret. The code tells these apart, and the status tells a refusal from a failure of
  ...
  class AuthorizationError < Error
    ...
    def self.from(error, default_message)
      new(error.description || error.code || default_message, code: error.code, status: error.status)
    end
