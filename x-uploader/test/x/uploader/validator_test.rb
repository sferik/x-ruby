...
module X
  class ValidatorTest < Minitest::Test
    ...
    def test_validate_extension
      assert_nil Uploader.const_get(:Validator).validate_extension!("avatar.png", %w[jpg png])
    end

    def test_validate_extension_ignores_case
      assert_nil Uploader.const_get(:Validator).validate_extension!(Pathname("AVATAR.PNG"), %w[jpg png])
    end
    ...
    def test_validate_media_category
      assert_equal "tweet_image", Uploader.const_get(:Validator).validate_media_category!("tweet_image")
    end

    def test_validate_amplify_video
      assert_equal "amplify_video", Uploader.const_get(:Validator).validate_media_category!("amplify_video")
    end
    ...
    def test_validate_media_category_takes_a_symbol
      assert_equal %w[tweet_video tweet_image], [:tweet_video, :TWEET_IMAGE].map { |category| Uploader.const_get(:Validator).validate_media_category!(category) }
    end

    def test_validate_media_category_raises_for_a_symbol_that_names_no_category
      error = assert_raises(ArgumentError) { Uploader.const_get(:Validator).validate_media_category!(:bogus) }
  ...
  class ValidatorUploadTest < Minitest::Test
    ...
    def test_validate_alt_text
      ...
      assert_nil Uploader.const_get(:Validator).validate_alt_text!("A")
      assert_nil Uploader.const_get(:Validator).validate_alt_text!("A" * 1000)
    end
    ...
    def test_validate_alt_text_raises_for_alt_text_longer_than_the_api_takes
      error = assert_raises(ArgumentError) { Uploader.const_get(:Validator).validate_alt_text!("A" * 1001) }
