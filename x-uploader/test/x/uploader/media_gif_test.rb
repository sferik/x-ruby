...
module X
  class MediaGifTest < Minitest::Test
    ...
    MAX_SIMPLE = Uploader::Media::MAX_SIMPLE_UPLOAD_BYTES
    ...
    def test_a_gif_no_larger_than_a_single_request_takes_uploads_whole
      with_gif(MAX_SIMPLE) { |path| Uploader::Media.upload(path, client: @client, media_category: "tweet_gif") }
    ...
    def test_a_gif_larger_than_a_single_request_takes_uploads_in_chunks
      with_gif(MAX_SIMPLE + 1) { |path| Uploader::Media.upload(path, client: @client, media_category: "tweet_gif") }
    ...
    def test_a_direct_message_gif_larger_than_a_single_request_takes_uploads_in_chunks
      with_gif(MAX_SIMPLE + 1) { |path| Uploader::Media.upload(path, client: @client, media_category: "dm_gif") }
    ...
    def test_a_small_animated_gif_of_its_own_category_uploads_whole
      Uploader::Media.upload("test/sample_files/sample_animated.gif", client: @client)
    ...
    def test_a_gif_uploads_in_chunks_only_once_a_single_request_cannot_take_it
      with_gif(MAX_SIMPLE) do |path|
        assert_equal [false, false], %w[tweet_gif dm_gif].map { |category| Uploader::Media.chunked_upload?(path, category) }
      end
      with_gif(MAX_SIMPLE + 1) do |path|
        assert_equal [true, true], %w[tweet_gif dm_gif].map { |category| Uploader::Media.chunked_upload?(path, category) }
      end
    end

    def test_a_video_and_subtitles_upload_in_chunks_whatever_their_size
      assert_equal [true] * 4, %w[amplify_video dm_video tweet_video subtitles].map { |category| Uploader::Media.chunked_upload?("nope.mp4", category) }
    end

    def test_an_image_uploads_whole_whatever_its_size
      with_gif(MAX_SIMPLE + 1) do |path|
        assert_equal [false, false], %w[tweet_image dm_image].map { |category| Uploader::Media.chunked_upload?(path, category) }
      end
    end

    def test_the_category_of_a_chunked_upload_is_read_in_any_case
      assert_equal [true, true, false], [Uploader::Media.chunked_upload?("nope.mp4", :TWEET_VIDEO),
        Uploader::Media.chunked_upload?("nope.mp4", "Tweet_Video"), Uploader::Media.chunked_upload?("test/sample_files/sample_animated.gif", :TWEET_GIF)]
    end
