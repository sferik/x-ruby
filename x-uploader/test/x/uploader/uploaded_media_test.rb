...
module X
  class UploadedMediaTest < Minitest::Test
    ...
    def media_in(state, **info)
      Uploader::UploadedMedia.new(ATTRS.merge("processing_info" => {"state" => state, **info.transform_keys(&:to_s)}))
    end
    ...
    def test_media_with_null_processing_information_is_ready
      media = Uploader::UploadedMedia.new({"id" => "7", "processing_info" => nil})
    ...
    def test_media_in_a_state_without_a_name_is_processing
      media = Uploader::UploadedMedia.new({"id" => "7", "processing_info" => {}})
    ...
    def test_is_frozen_with_all_that_it_holds
      media = Uploader::UploadedMedia.new({"id" => +"7", "processing_info" => {"state" => +"pending"}, "variants" => [+"a"]})
    ...
    def test_media_with_the_same_attributes_is_equal
      same = Uploader::UploadedMedia.new(ATTRS.dup)
    ...
    def test_media_is_equal_to_nothing_else
      other = Uploader::UploadedMedia.new(ATTRS.merge("size" => 1))

      assert_equal [false, false, false], [@media == other, @media == ATTRS, @media == Class.new(Uploader::UploadedMedia).new(ATTRS)]
      refute_equal @media.hash, other.hash
      refute_equal @media.hash, Class.new(Uploader::UploadedMedia).new(ATTRS).hash
    end

    def test_from_builds_media_from_data_and_nothing_from_none
      assert_equal @media, Uploader::UploadedMedia.from(ATTRS)
      assert_nil Uploader::UploadedMedia.from(nil)
    end

    def test_inspect_of_media_without_an_identifier_raises_nothing
      assert_equal "#<X::Uploader::UploadedMedia id=nil media_key=nil state=nil>", Uploader::UploadedMedia.new({}).inspect
    end
