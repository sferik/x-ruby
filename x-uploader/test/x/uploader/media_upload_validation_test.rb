...
module X
  class MediaUploadValidationTest < Minitest::Test
    ...
    def test_upload_rejects_a_chunk_size_that_is_not_positive_whatever_the_upload
      assert_raises(ArgumentError) { Uploader::Media.upload("test/sample_files/sample.png", client: @client, chunk_size_mb: 0) }
      assert_raises(ArgumentError) { Uploader::Media.upload("test/sample_files/sample.png", client: @client, concurrency: 0) }
      assert_not_requested :post, BASE_URL
    end

    def test_upload_rejects_alt_text_the_api_would_refuse_before_uploading_anything
      error = assert_raises(ArgumentError) { Uploader::Media.upload("test/sample_files/sample.mp4", client: @client, alt_text: "A" * 1001) }

      assert_equal "alt_text must be 1 to 1000 characters, not 1001", error.message
      assert_raises(ArgumentError) { Uploader::Media.upload("test/sample_files/sample.png", client: @client, alt_text: "") }
      assert_not_requested :post, "#{BASE_URL}/initialize"
      assert_not_requested :post, BASE_URL
    end
    ...
    def test_upload_describes_media_with_the_longest_alt_text_the_api_takes
      ...
      Uploader::Media.upload("test/sample_files/sample.png", client: @client, alt_text: "A" * 1000)
    ...
    def test_upload_takes_the_media_category_of_an_upload_in_a_single_request_as_a_symbol
      stub_upload_request
      Uploader::Media.upload("test/sample_files/sample.png", client: @client, media_category: :TWEET_IMAGE)
    ...
    def test_upload_binary_takes_the_media_category_as_a_symbol
      stub_upload_request
      Uploader::Media.upload_binary("GIF89a", client: @client, media_category: :TWEET_GIF)
    ...
    def test_upload_raises_for_a_media_category_that_names_none
      error = assert_raises(ArgumentError) { Uploader::Media.upload("test/sample_files/sample.png", client: @client, media_category: :bogus) }
