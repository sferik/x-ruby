...
* Raise `X::ResourceNotFound`, an `X::Error`, from `current_user`, `X::User.current!`, and the new `find!`, `find_user!`, `find_post!`, `find_list!`, `find_space!`, and `find_direct_message!`, so a missing resource is an error a caller can rescue in one place
...
* Return an `X::Uploader::UploadedMedia` from `upload`, `upload_binary`, `chunked_upload`, `await_processing`, and `await_processing!`, and from `upload_media`, `upload_media_binary`, and `await_media_processing` on the client, in place of a Hash: a frozen object that reads the media's `id`, as an Integer, `media_key`, `size`, `expires_at`, `processing_info`, `state`, and `check_after_secs`, tells `processing?`, `failed?`, and `ready?`, compares by its attributes, and still reads as the Hash with `[]`, `fetch`, `dig`, `key?`, and `to_h`, so `media["id"]` works as it did; the uploaders and `create_post` take it wherever they take media, and `X::Uploader::MediaProcessingFailed#status` and `X::Uploader::MediaProcessingTimeout#status` hold it
...
* Count the posts that match a query with `X::Post.count` and `X::Post.count_all`, or by period with `X::Post.counts` and `X::Post.counts_all`, and on a client with `count_posts`, `count_all_posts`, `post_counts`, and `post_counts_all`, which have tweet-named aliases; a client that signs with OAuth 1.0a counts with a copy that authenticates as the app, since the counts endpoints refuse OAuth 1.0a
...
* Keep connections open between requests to the same host, up to eight idle per host and for the 30 seconds of `keep_alive_timeout`, a setting of `X::Client` and `X::Connection`, which X holds an idle connection well beyond, and which a client behind a proxy that closes idle connections sooner can lower, so a request no longer opens a TCP and TLS connection of its own; a connection whose request fails is closed, `X::Connection#close` closes the idle ones, a change of proxy or debug output opens new ones, and a forked process opens its own and leaves its parent's open when it closes them; `X::Client#close` closes the connections a client keeps open, and those of its app-only copy, and a later request opens one again
* Authenticate as the app with `X::Client#app_only`, a copy of a client that signs with OAuth 1.0a, which fetches the app's bearer token once and reuses it, and which the client returns again, with the connections it keeps open, until the client's credentials or settings change, when it closes the connections of the copy it replaces; threads that ask for the copy together get one copy, and fetch one token
