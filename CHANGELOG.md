...
* Hydrate the stubs of a page together, in one batch lookup for the whole page rather than one request per stub, so walking `user.followers.stubs` costs a request per page; the stubs of lists, communities, and direct messages, which the API cannot look up in batches, hydrate one at a time, and `refresh` looks a stub up again on its own rather than read what the lookup of its page found
