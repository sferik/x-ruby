...
* Authenticate as the app with `X::Client#app_only`, a copy of a client that signs with OAuth 1.0a, which fetches the app's bearer token once and reuses it, and which the client returns again, with the connections it keeps open, until the client's credentials or settings change
